...
# Create the parser
parser = argparse.ArgumentParser(description="Generate NFT assets")

# Add the arguments
parser.add_argument('RequestFile', metavar='requestfile', type=str, help='the .json file with the request')

# Execute the parser
args = parser.parse_args()

# Load the JSON file
with open(args.RequestFile, 'r') as file:
...
for image_file in os.listdir(timestamp):
    if image_file.startswith("final_"):
        ...
        uploaded_files.append(file_cid)
        ...
        logging.debug('Uploaded file URL: %s', json_ipfs_url)
...
print(json.dumps({"urls": uploaded_files}))
